...
import type { RecoveryFlow, UiText } from '@ory/client'
...
function RecoveryPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await submitRecoveryFlow(flow.id, formData)
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-slate-50">
        <Alert variant="destructive" className="max-w-md">
          <AlertDescription>{error || 'Failed to load recovery form'}</AlertDescription>
        </Alert>
      </div>
    )
  }
