...
export function listLibrary(accessToken: string) {
	return apiRequest<LibraryItemWithSource[]>('/api/library/items/with-sources?limit=50', { accessToken });
}
...
export function addLibraryItem(accessToken: string, sourceID: string) {
	return apiRequest<LibraryItem>('/api/library/items', {
		method: 'POST',
		accessToken,
		body: JSON.stringify({ source_id: sourceID, status: 'to_consume', visibility: 'private' }),
	});
}
...
export function listPublicLibrary(username: string) {
	return apiRequest<LibraryItemWithSource[]>(`/users/${encodeURIComponent(username)}/library/with-sources?limit=50`);
}
