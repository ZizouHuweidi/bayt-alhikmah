...
func (h *Handler) RegisterRoutes(g *echo.Group) {
	...
	g.GET("/auth/:provider", h.OAuthLogin)
	g.GET("/auth/:provider/callback", h.OAuthCallback)
}
...
// @Description Redirects user to the OAuth provider for login
// @Tags auth
// @Param provider path string true "OAuth Provider (e.g., google)"
// @Success 302
// @Router /auth/{provider} [get]
func (h *Handler) OAuthLogin(c echo.Context) error {
	// We need to set the provider in the context for gothic
...
// @Description Handle callback from OAuth provider and return tokens
// @Tags auth
// @Param provider path string true "OAuth Provider (e.g., google)"
// @Success 200 {object} response.Response{data=jwt.TokenPair}
// @Failure 401 {object} response.Response
...
func (h *Handler) OAuthCallback(c echo.Context) error {
	...
	if err != nil {
		return json.Unauthorized(c, err.Error())
	}

	tokens, err := h.userService.LoginOrRegisterOAuth(c.Request().Context(), gothUser.Email, gothUser.Name, provider, gothUser.UserID)
	if err != nil {
		return json.InternalServerError(c, err)
	}
