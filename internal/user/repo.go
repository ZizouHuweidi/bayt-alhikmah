...
func (r *repository) Create(ctx context.Context, user *User) error {
	query, args, err := r.sb.Insert("users").
		Columns("email", "username", "password_hash", "provider", "provider_id").
		Values(user.Email, user.Username, user.PasswordHash, user.Provider, user.ProviderID).
...
func (r *repository) GetByProvider(ctx context.Context, provider, providerID string) (*User, error) {
	var user User
	query, args, err := r.sb.Select("*").From("users").
